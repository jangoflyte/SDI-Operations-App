...
const getTemplate = async () => {
  let template = await knex('template').select('*')

  for (let t of template) {
    t.post_array = JSON.parse(t.post_array)
  }
  return template
}
...
const postPosition = async req => {
  ...
  if (
    req.body.post_array &&
    Array.isArray(req.body.post_array) &&
    req.body.template === ''
  ) {
    let result = []
    ...
    return result[0]
  } else if (Number.isInteger(req.body.template)) {
    let workingTemplate = await knex('template')
      .select('*')
...
const postNotification = async ({ userId, notification }) => {
  ...
  if (userId === 'all') {
    ...
    let allAdmins = await knex('user_table').select('id').where({ admin: true })
